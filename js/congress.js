...
function generar_tabla(arrayTablaBD, sitio) {
    ...
    // B. 10. ESCRIBIR (CON innerHTML) LOS CONTENIDOS DE LOS CAMPOS EN LA HILERA DE REGISTRO RECIEN CREADA
            ...
            if (cuentaCampos == 4) {
                ...
                // pretendemos hacer esto con el DOM.....:
                //  <div class="progress" style="background-color: wheat">
                //      <div class="progress-bar progress-bar-striped " role="progressbar" style="width: 50%; background-color:darkred; 
                //                                                        color: wheat" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100">
                //          25% votes
                //      </div>
                //  </div>

                // debugger;
                //  lightgrey  darkred  wheat
                var barraProgresoFondo = document.createElement("div");
                barraProgresoFondo.className = "progress";
                barraProgresoFondo.style.backgroundColor = "lightgrey";  // lightgrey
                //barraProgresoFondo.backgroundColor = "wheat";
                barraProgresoFondo.setAttribute("aria-valuemin", "0");
                barraProgresoFondo.setAttribute("aria-valuemax","100");

                    var barraProgresoDentro = document.createElement("div");
                    barraProgresoDentro.className = "progress-bar progress-bar-striped";
                    barraProgresoDentro.setAttribute("role", "progressbar");
                    barraProgresoDentro.style.backgroundColor = "darkred";  // darkred
              
                    // label con el valor encima
                    barraProgresoDentro.color = "wheat";    // wheat
                    //barraProgresoDentro.value=(arrayTablaBD[cuentaCampos][cuentaRegistros+1]);                        
                    var pct = (arrayTablaBD[cuentaCampos][cuentaRegistros+1]);
                    barraProgresoDentro.style.width = `${pct}%`
                    var max = barraProgresoFondo.getAttribute("aria-valuemax");
                    barraProgresoDentro.setAttribute("aria-valuemin", 0);
                    barraProgresoDentro.setAttribute("aria-valuenow", pct);
                    barraProgresoDentro.setAttribute("aria-valuemax",max);                        
                    barraProgresoDentro.innerHTML = pct+"%";
                    barraProgresoFondo.appendChild(barraProgresoDentro);

                celda.appendChild(barraProgresoFondo);

                 // "#8b0000";  // color darkred
                //barraProgreso.max = 100;
                //barraProgreso.value=(arrayTablaBD[cuentaCampos][cuentaRegistros+1]);
                //switch (barraProgreso.value){
                //    case (this<20):
                //}                
                // barraProgreso.className="progress-bar progress-bar-striped bg-warning" // color naranja por defecto
                // if (barraProgreso.value <20){barraProgreso.className = "bg-danger"}    // color rojo
                // if (barraProgreso.value >80){barraProgreso.className = "bg-success"}   // color verde            
            ...
            if ((cuentaRegistros==0) || (cuentaRegistros==15) || (cuentaRegistros==20)) {
                switch (cuentaCampos){
                    ...
                    case 1 : New_part = (textoCelda.textContent=="R")?"republican":"democrat";  // partido del senador
                        break;
                    case 4 : New_vots = barraProgresoDentro.value;  //textoCelda.textContent;
                }   
